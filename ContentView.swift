...
struct ContentView: View {
    ...
    @State var submitAlert: Bool = false
    @State var showHelpAgain: Bool = false
    ...
    var body: some View {
        //colocar uma diva no mesmo lugar que outra crasha
        NavigationStack {
            ZStack {
...
//                    .position(x: 100, y: 1080)
    //                    .background(Color.red.opacity(0.8).frame(width: 150, height: 50))
    //                    .clipShape(.capsule.frame(width: 150, height: 50))
                    .shadow(radius: 2.5)
                    .tint(.white)
    ...
    //                                        }
            ...
            //                                        print(coordsX)
                    ...
                    .alert(resultSubmit, isPresented: $submitAlert) {
                        if isSolved == false {
                            Button("Try again?") {
...
//                        .position(x: 1120, y: 1080)
                ...
                HStack {
                    Button("Clean", systemImage: "arrow.trianglehead.2.counterclockwise") {
                        UIDevice.vibrate()
                        coordsX.removeAll()
                        queens.removeAll()
                        colisions.removeAll()
